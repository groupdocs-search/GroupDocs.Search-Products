...
steps:
    enable: true
    title: "TXTドキュメントでのブール検索の手順"
    content: |
      [GroupDocs.Search](/search/java/)は、TXTドキュメント内で簡易なテキスト検索を可能にします。ブール条件をサポートすることで、Javaアプリケーションにおける検索精度を向上させることができます。
      
      1. 検索インデックスを保存するフォルダを指定します。
      2. TXTドキュメントを含むフォルダを選択します。
...
more_features:
  ...
  code_samples_ext:
    ...
    - title: "複雑なブール検索クエリの使用"
      content: |
        この例では、TXTファイルでのブール検索を実行する方法を示します。
      code:
        title: "Java"
        content: |
          ```java {style=abap}
