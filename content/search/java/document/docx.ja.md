...
############################# Head ############################
head_title: "DOCX ドキュメント内のテキストを見つける GroupDocs.Search for Java"
...
steps:
    enable: true
    title: "DOCX ファイル内でのテキスト検索を実行する"
    content: |
      [GroupDocs.Search](/search/java/) は洗練されたロジックとインデックス作成を用いて DOCX ファイルを検索し、Java アプリケーション内の検索精度を向上させます。
      
      1. 検索インデックスを保存するディレクトリを設定します。
      2. DOCX ファイルが含まれるフォルダを選択します。
      3. 追加の検索オプションを定義します。
      4. 検索を実行し、結果を分析します。
...
more_features:
  ...
  code_samples_ext:
    ...
    - title: "高度な検索クエリの実装"
      content: |
        この例では、DOCX ドキュメント内を効率的に検索するための検索クエリの使用方法を示します。
      code:
        title: "Java"
        content: |
          ```java {style=abap}
...
more_formats:
    enable: true
    title: "GroupDocs.Search を使用して DOCX ドキュメント全体の情報を見つける"
    exclude: "DOCX"
