...
draft: false
path: "ko/search/net/document/dotx/"
...
############################# Header ############################
title: "C# .NET API를 통해 DOTX에 인스턴트 문서 검색을 추가하는 방법 "
...
steps:
    enable: true
    block:
    - title_left: ".NET API를 통해 DOTX 문서에 대한 검색 색인 생성"
      content_left: |
    ...
    - title_left: "검색 색인에 동기적으로 DOTX 문서를 추가하는 방법"
      content_left: |
    ...
    - title_left: ".NET을 통해 비동기식으로 문서 인덱싱 수행"
      ...
      title_right: "C#을 통한 비동기식 DOTX 문서 인덱싱"
      content_right: |
        * 먼저 인덱스 폴더의 경로를 지정해야 합니다.
        * 검색할 문서가 있는 폴더의 경로 지정
        * [Index(indexFolder)](https://apireference.groupdocs.com/search/net/groupdocs.search.indexrepository/search/methods/2) 클래스의 인스턴스 생성
    ...
    - title_left: "DOTX Docs .NET에서 검색 결과를 사용하고 강조 표시하는 방법"
      content_left: |
      ...
      title_right: "C#을 통해 DOTX 파일에서 검색 결과 강조 표시 "
      content_right: |
        * 색인에서 검색 수행
        * 검색 성공 후 결과 출력
        * 문서를 반복하고 찾은 문서 표시
